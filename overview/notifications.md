...
Special measures are needed to make it impossible to use the notification
...
must be the node's *primary name*, i.e. the name reported by the node
itself.

In response to a notification, the receiver may indicate that it does
...
This makes the UI more responsive. The node begins to send
...
and the client, when it reconnects, reports the timestamp of the last
